...
const generateInquiryEmailText = (inquiry, property) => {
    const formatPhoneForWhatsApp = (phone) => {
        let cleanPhone = phone.replace(/\D/g, '');
        if (cleanPhone.startsWith('91') && cleanPhone.length > 10) {
            cleanPhone = cleanPhone.substring(2);
...
const sendInquiryNotification = async (inquiry, property) => {
    try {
        ...
        const mailOptions = {
            from: '"South Delhi Realty" <' + config.from + '>',
            to: config.from,
            subject: subject,
...
const sendTestEmail = async () => {
    try {
        ...
        await (0, exports.sendInquiryNotification)(testInquiry);
        return {
            success: true,
...
const generateUserConfirmationEmailHTML = (inquiry, property) => {
    const propertyPageLink = property ?
        `${(process.env.CLIENT_URL || 'https://southdelhirealty.com').replace(/\/$/, '')}/property/${property.slug || property.id}` :
        null;
...
const generateUserConfirmationEmailText = (inquiry, property) => {
    const propertyPageLink = property ?
        `${(process.env.CLIENT_URL || 'https://southdelhirealty.com').replace(/\/$/, '')}/property/${property.slug || property.id}` :
        null;
    const propertySection = property ? `
...
const sendUserConfirmationEmail = async (inquiry, property) => {
    try {
        ...
        const mailOptions = {
            ...
            replyTo: 'admin@southdelhirealty.com'
        };
